...
std::shared_ptr<ast::Statement> Parser::ParseSetStatement() {
  ...
  if (stmt->target_->String() == "mixer") {
    ...
    NextToken();  // cur = [
    NextToken();  // cur = first token of index expression
    stmt->buf_idx_expr_ = ParseExpression(Precedence::LOWEST);
    if (!ExpectPeek(token::SLANG_RBRACKET)) {
      std::cerr << "Expected ] after buf index\n";
      return nullptr;
    }
    if (!ExpectPeek(token::SLANG_COLON)) {
      std::cerr << "Expected : after buf[...]\n";
      return nullptr;
    }
...
std::shared_ptr<ast::Expression> Parser::ParseForPrefixExpression() {
  ...
  else if (cur_token_.type_ == token::SLANG_IMAGESOUNDER)
    return ParseImageSounderExpression();
...
std::shared_ptr<ast::Expression> Parser::ParseImageSounderExpression() {
  auto imagesounder = std::make_shared<ast::ImageSounderExpression>(cur_token_);
...
std::shared_ptr<ast::Expression> Parser::ParseInfixExpression(
  ...
  if (!IsPostfixOperator(cur_token_.type_)) {
